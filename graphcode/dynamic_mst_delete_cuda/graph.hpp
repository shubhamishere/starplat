...
public:
  ...
  { 
    ...
    int32_t *d_edgeList;int32_t *d_edgeLen;
    int32_t *d_rev_indexofNodes;
    int32_t *d_srcList;int32_t *d_srcLen;
    int32_t *d_diff_indexofNodes;
    int32_t *d_diff_edgeList;int32_t *d_diff_edgeLen;
    int32_t *d_diff_rev_indexofNodes;
    int32_t *d_diff_rev_edgeList;int32_t *d_diff_rev_edgeLen;

    // allocate memory on device

    cudaMalloc(&d_batch, sizeof(update) * (size));
    cudaMalloc(&d_indexofNodes, sizeof(int32_t) * (V + 1));
    cudaMalloc(&d_edgeList, sizeof(int32_t) * (E));cudaMalloc(&d_edgeLen, sizeof(int32_t) * (E));
    cudaMalloc(&d_rev_indexofNodes, sizeof(int32_t) * (V + 1));
    cudaMalloc(&d_srcList, sizeof(int32_t) * (E));cudaMalloc(&d_srcLen, sizeof(int32_t) * (E));
    cudaMalloc(&d_diff_indexofNodes, sizeof(int32_t) * (V + 1));
    cudaMalloc(&d_diff_edgeList, sizeof(int32_t) * (diffE));cudaMalloc(&d_diff_edgeLen, sizeof(int32_t) * (diffE));
    cudaMalloc(&d_diff_rev_indexofNodes, sizeof(int32_t) * (V + 1));
    cudaMalloc(&d_diff_rev_edgeList, sizeof(int32_t) * (diffRevE));cudaMalloc(&d_diff_rev_edgeLen, sizeof(int32_t) * (diffRevE));
    ...
    //
    cudaMemcpy(d_srcLen, rev_edgeLen, sizeof(int32_t) * (E), cudaMemcpyHostToDevice);
    ...
    // call kernel
    del_edges_CSR<<<numBlocks, numThreads>>>(size, d_batch, d_indexofNodes, d_edgeList,d_edgeLen,
                                             d_rev_indexofNodes, d_srcList,d_srcLen);
    if (diff_edgeList != NULL)
      del_edges_diffCSR<<<numBlocks, numThreads>>>(size, d_batch,
                                                   d_diff_indexofNodes, d_diff_edgeList, d_diff_edgeLen,
    ...
    cudaMemcpy(rev_edgeLen, d_srcLen, sizeof(int32_t) * (E), cudaMemcpyDeviceToHost);
