...
class graphstatic
{
  ...
  int32_t* edgeLen;
  char* filePath;
  std::map<int32_t,std::vector<sedge>> edges;
  ...
  std::map<int,std::vector<sedge>> getEdges()
  {
      return edges;
  }
  ...
  void parseGraph(bool isWeighted = false)
  {
   ...
   //  printf("Here half\n");
    ...
    #pragma omp parallel for
     ...
     {
       std::vector<sedge>& edgeOfVertex=edges[i];

       sort(edgeOfVertex.begin(),edgeOfVertex.end(),
                            [](const sedge& e1,const sedge& e2) {
