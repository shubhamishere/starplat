...
__global__ void update_leader(int * offset, int * edgeList, int * diff_offset, int * diff_edgeList, int * component, int  n,bool *d_isMSTEdge, bool *d_diff_isMSTEdge){ 
...
void colourkernel( int * offset, int * edgeList, int * diff_offset, int * diff_edgeList, int * component, int  n,bool * d_isMSTEdge, bool *d_diff_isMSTEdge)
{
  ...
  {   
    ...
    update_leader<<<numBlocks,numThreads>>>(offset,edgeList,diff_offset,diff_edgeList,component,n,d_isMSTEdge,d_diff_isMSTEdge);
...
while(!noNewComp) {
  ...
  Boruvka_kernel_1<<<numBlocks, threadsPerBlock>>>( V,d_color);
  colourkernel(d_offset,d_edgeList,d_diff_offset,d_diff_edgeList,d_color,V,d_isMST,d_diff_isMST);
  cudaDeviceSynchronize();
...
int  main( int  argc, char** argv) {
  ...
  long long int reval = Recalculate(G1,updatesinp);
  cudaDeviceSynchronize();
  graph G2(updatedgraph,"cuda",true);
  G2.parseGraph();
  long long int eval = Boruvka(G2);
  if(reval!=eval){
