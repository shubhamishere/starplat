...
__global__ void applyMovesKernel(int *P, int *d_best_community, float *d_beta, float *d_k_x, int numVertices);
...
__global__ void applyMovesKernel(int *P, int *d_best_community, float *d_beta, float *d_k_x, int numVertices)
{
    ...
    int best_community = d_best_community[tid];
    if (best_community == -1) return; // No move needed

    int initial_community = P[tid];
    float k_x = d_k_x[tid];

    // Update the community
    P[tid] = best_community;

    // Update d_beta safely
    atomicAdd(&d_beta[initial_community], -k_x);
    atomicAdd(&d_beta[best_community], k_x);
}
...
void moveNodes(Graph &G, Partition &P, int &updated)
{
    ...
    do
    {
        moveNodesKernel<<<blocksPerGrid, threadsPerBlock>>>(G.d_row_ptr, G.d_col_idx,
                                                            ...
                                                            G.d_alpha, G.d_beta, d_best_community, d_k_x,
                                                            G.num_nodes);
        cudaDeviceSynchronize();

        // Step 2: Apply the moves
        applyMovesKernel<<<blocksPerGrid, threadsPerBlock>>>(P.d_community, d_best_community, G.d_beta, d_k_x, G.num_nodes);
        cudaDeviceSynchronize();
